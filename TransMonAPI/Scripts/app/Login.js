var app = angular.module("IPSLLogin", []);
       app.controller("LoginCtrl", function ($scope, $location, $http, $window) {
        ...
        //$scope.myUrl = $location.absUrl();
           function Go(username, useremail, bAdmin, bUser, bCode,bIPSLAdmin) {
              ...
              //alert("Login response:"+response.data.status);
                if (response.data.status == "00" || response.data.status == "54") {
                   ...
                   // alert(response.data.bankUser);
                    Go(response.data.firstname + " " + response.data.lastname, response.data.email, response.data.bankAdmin, response.data.bankUser, response.data.bankCode,response.data.ipslAdmin);
