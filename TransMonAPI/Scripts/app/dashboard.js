        ...
        $scope.openModal = function (name, code) {
            ...
            for(var i=0;i<response.data.FailureRateList.length;i++)
            {
                $scope.jsonArray[i]={
                    ...
                    x:response.data.FailureRateList[i].Date*1000,
         ...
         $scope.jsonArray2 = [];
            for(var i=0;i<response.data.FailureRateListIn.length;i++)
            {
                $scope.jsonArray2[i]={
                    ...
                    x:response.data.FailureRateListIn[i].Date*1000,
       ...
       /* labels: {
            ...
            for(var i=0;i<response.data.FailureRateListVolumeIn.length;i++)
            {
                $scope.jsonArrayV1[i]={
                    ...
                    x:response.data.FailureRateListVolumeIn[i].Date*1000,
         ...
         $scope.jsonArrayV2 = [];
            for(var i=0;i<response.data.FailureRateListVolumeOut.length;i++)
            {
                $scope.jsonArrayV2[i]={
                    ...
                    x:response.data.FailureRateListVolumeOut[i].Date*1000,
...
$scope.NewloadPage2 = function (myData) {           
 ...
 function PageloadStarting() {           
            ...
            $http({
                method: 'POST',               
                url: 'https://tpms.ipsl.co.ke:4433/TransMonAPI/api/PageLoadBankData',                
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                withCredentials: false,
            }).then(function successCallback(response) {
                $scope.blackout = false;
